...
export const adminService = {
  getDashboardStats: async () => {
    ...
    const [
      ...
      prisma.order.aggregate({
        where: { paymentMethod: 'COD', paymentStatus: 'PENDING', status: { notIn: ['CANCELLED', 'RETURNED'] } },
        _sum: { totalAmount: true },
      }),
      prisma.order.aggregate({
        where: { paymentMethod: 'COD', paymentStatus: 'COMPLETED' },
        _sum: { totalAmount: true },
      }),
    ...
    return {
      ...
      cashPayment: {
        pendingCodAmount: codPendingResult._sum.totalAmount || 0,
        collectedCodAmount: codCollectedResult._sum.totalAmount || 0,
        totalCodOrders: await prisma.order.count({ where: { paymentMethod: 'COD' } }),
      },
      products: {
        total: totalProducts,
  ...
  getQuickOptions: async () => {
    ...
    return {
      ...
      cashPaymentOptions: {
        totalCodOrders: await prisma.order.count({ where: { paymentMethod: 'COD' } }),
        pendingCodOrders: await prisma.order.count({ where: { paymentMethod: 'COD', paymentStatus: 'PENDING' } }),
        pendingCodAmount: (await prisma.order.aggregate({ where: { paymentMethod: 'COD', paymentStatus: 'PENDING' }, _sum: { totalAmount: true } }))._sum.totalAmount || 0,
        collectedCodAmount: (await prisma.order.aggregate({ where: { paymentMethod: 'COD', paymentStatus: 'COMPLETED' }, _sum: { totalAmount: true } }))._sum.totalAmount || 0,
      },
      quickActions: [
