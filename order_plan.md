...
Viewed order_plan.md:1-20
Viewed order_plan.md:1-200
Viewed order.service.ts:250-265
Viewed order.service.ts:435-455
Edited order.service.ts
Ran command: `cmd /c "npx jest tests/order.test.ts"`
Used tool: manage_task
Viewed order_plan.md:1-50
Edited order_plan.md
Used tool: manage_task

`order_plan.md` has been updated with the complete order lifecycle, COD payment auto-completion, and Mobile Banking transaction verification rules.

---

### **Key Updates in [order_plan.md](file:///i:/supermart-api/order_plan.md)**

1. **Order Status Progression Flow & Rules (Section 1)**:
   - **Cash on Delivery (COD)**:
     - When COD payment is recorded (`paymentMethod: "COD"` via `POST /api/v1/orders/:id/pay`), `paymentStatus` is set to `COMPLETED` and `status` automatically transitions to `COMPLETED`.
     - Automatic notifications sent to Admin and User.
   - **Mobile Banking Payment**:
     - Submitting a mobile banking payment (`bKash`, `Rocket`, `Nagad`, `Card`, `Bank Transfer`) attaches `transactionId` with `paymentStatus: "PENDING"` and sends a high-priority notification to Admin.
     - **Admin Verification (`POST /api/v1/orders/:id/verify-payment`)**:
       - **Valid (`isValid: true`)**: Sets `paymentStatus` = `COMPLETED`, `status` = `COMPLETED`, and notifies User.
       - **Invalid (`isValid: false`)**: Sets `paymentStatus` = `FAILED`, `status` = `CANCELLED`, restores product inventory stock, and notifies User with the reason.
   - **Return Request**: Allowed for orders that are `DELIVERED` or `COMPLETED`.

2. **Admin Panel Endpoint & Request/Response Specification (Section 4.5)**:
   - Exact HTTP method (`POST /api/v1/orders/:id/verify-payment`), request headers, body payloads (`{ "isValid": true }` / `{ "isValid": false, "reason": "..." }`), and response bodies.

3. **Frontend API Client Helper (`orderApiClient.ts`, Section 5)**:
   - Added `verifyPayment(adminToken, orderId, isValid, reason)` method.

Viewed order_plan.md:3-24

All **14 integration tests** in `tests/order.test.ts` passed with **100% pass rate**!

### **Verified Features & Integration Status**

1. **Cash on Delivery (COD) Auto-Completion**:
   - Recording COD payment (`paymentMethod: "COD"`) updates `paymentStatus` to `COMPLETED` and automatically sets `status` to `COMPLETED`.
   - Notifications dispatched to Admin and Customer.

2. **Mobile Banking Payment & Admin Transaction Verification**:
   - Mobile banking payments (`bKash`, `Rocket`, `Nagad`, `Card`, `Bank Transfer`) attach `transactionId` with `paymentStatus: "PENDING"` and alert Admin.
   - **Admin Verification Endpoint**: `POST /api/v1/orders/:id/verify-payment` (Requires `ADMIN` role)
     - **Valid TxnID (`{ "isValid": true }`)**: `status` = `COMPLETED`, `paymentStatus` = `COMPLETED`.
     - **Invalid TxnID (`{ "isValid": false, "reason": "..." }`)**: `status` = `CANCELLED`, `paymentStatus` = `FAILED`, inventory stock restored automatically.

3. **Documentation Updated**:
   - [order_plan.md](file:///i:/supermart-api/order_plan.md#L7-L35): Full progression flow, payment rules, endpoint payloads, and `orderApiClient.ts` helper methods.
...
8. COMPLETED        -> Order accepted & finalized (COD payment automatically completes)
